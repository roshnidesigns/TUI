/*
 * Social Battery — the motorized fader, plus one servo arm (D7), hardcoded demo
 * Tangible User Interface, CIID
 *
 * Based on FINAL-DIFF-SPEED, with one more deliberate change: holding the slider
 * still for HAND_HOLD_MS (2s) while it's mid-swing now counts as a hand and
 * captures the position, even with zero active pushback. Previously, holding it
 * still (not pushing back) fell into the stall/boost friction logic instead - the
 * motor would wind up torque against your hand for ~1.5s before giving up and
 * trying the other end, never actually releasing to capture your hold. See the
 * comment at HAND_HOLD_MS's use (in the SWINGING case) for the tradeoff this
 * knowingly accepts - it can also mistake a genuine mechanical stall for a hand.
 *
 * Everything else carries over from FINAL-DIFF-SPEED unchanged: the fader drives
 * continuously toward the real physical end of the swing (no virtual paced target,
 * smooth glide like Fader_Mirror), and the servo arm still runs its own hardcoded
 * cycle on its own from the moment the board boots - OFF -> LOW -> MED -> HIGH,
 * 20 seconds each, then loops back to OFF and repeats. Typing a manual arm command
 * (low/med/high/stop/go) still works and takes over from the cycle.
 *
...
// Hardcoded demo cycle: runs the arm through OFF -> LOW -> MED -> HIGH, 20s each,
// on repeat, starting the moment the board boots - no serial input needed. Any
// manual arm command (low/med/high/stop/go) takes over and cancels it.
enum CycleStep { CYCLE_OFF = 0, CYCLE_LOW = 1, CYCLE_MED = 2, CYCLE_HIGH = 3, CYCLE_STEP_COUNT = 4 };
const char* CYCLE_NAME[CYCLE_STEP_COUNT] = { "OFF", "LOW", "MED", "HIGH" };
#define ARM_CYCLE_DWELL_MS 20000   // 20 seconds per step
bool armAutoCycle = true;
uint8_t cycleStep = 0;
unsigned long lastCycleSwitch = 0;   // 0 = hasn't started its first step yet
...
// Drives the arm into one step of the hardcoded demo cycle.
void enterCycleStep(uint8_t step) {
  switch (step) {
    case CYCLE_OFF:  armRunning = false; armHeld = false; break;   // eases back to ARM_CENTER_ANGLE and holds
    case CYCLE_LOW:  startArm(ARM_LOW);  break;
    case CYCLE_MED:  startArm(ARM_MED);  break;
    case CYCLE_HIGH: startArm(ARM_HIGH); break;
  }
  Serial.print("ARM CYCLE -> ");
  Serial.println(CYCLE_NAME[step]);
}
...
// Nudged up slightly from 222/226/230 - the jamming risk was at the floor, not here.
//                            Off  Low  Medium  High
const int SWING_SPEED[4] = {   0,  226,   230,   234 };            // PWM duty per level, 0-255
...
// Widened from 2: that was tight enough that ordinary motor/ADC noise kept reading
// as "moving again" even while genuinely held still, resetting the HAND_HOLD_MS
// stillness clock every pass and never letting it accumulate - the hand-hold never
// fired because of this, not because the 2-second logic itself was wrong.
#define STALL_NOISE 6       // counts of change that do not count as progress
#define BOOST_MAX 60        // never wind past this - past here, more force won't help either way

// How long the slider can sit with zero real progress, while being driven, before
// it's treated as a hand holding it in place rather than friction. See the comment
// where this is used (in the SWINGING case) for the tradeoff this accepts.
#define HAND_HOLD_MS 2000
...
void handleCommand(char* cmd) {
  ...
  if (eqIgnoreCase(cmd, "c")) {
    ...
    armAutoCycle = false;   // a manual arm command takes over from the hardcoded cycle
    startArm(ARM_LOW);
...
void setup() {
  ...
  Serial.println("ARM auto-cycling OFF -> LOW -> MED -> HIGH, 20s each, repeating");
  Serial.print("travel ");
...
void loop() {
  ...
  if (armAutoCycle && (lastCycleSwitch == 0 || cycleNow - lastCycleSwitch >= ARM_CYCLE_DWELL_MS)) {
    if (lastCycleSwitch != 0) cycleStep = (cycleStep + 1) % CYCLE_STEP_COUNT;   // advance to the next step, wrapping HIGH -> OFF
    lastCycleSwitch = cycleNow;
    enterCycleStep(cycleStep);
  }
    ...
    case SWINGING: {
      ...
      if (gap > MARGIN_ERROR) {                                  // not yet close enough to call it arrived
        if (abs(sliderVal - stallRef) > STALL_NOISE) {
          ...
          stallRef = sliderVal;
          stallTime = millis();
          holdTime = millis();                     // real progress - reset the stillness clock
          ...
          stallTime = millis();
          if (boost < BOOST_MAX) {
            boost += STALL_STEP;      // try a bit more duty next pass, in case it is just friction
          }
        }

        // DELIBERATE CHANGE from FINAL-DIFF-SPEED: held still (no real progress, at any
        // boost level) for HAND_HOLD_MS is now treated as a hand, not friction - capture
        // right here instead of continuing to push against it. holdTime only resets on
        // genuine progress above, so this measures continuous stillness, independent of
        // the boost ramp.
        //
        // This reopens a bug the previous design deliberately avoided (see git history /
        // FINAL-DIFF-SPEED's comment here): a genuine mechanical stall - friction, a hard
        // stop, not a hand - held this long would ALSO now re-level from wherever it got
        // stuck, often an extreme. Accepted knowingly: HAND_HOLD_MS is long enough (2s)
        // that a real stall should already have broken free via the boost ramp above.
        if (pastGrace && millis() - holdTime >= HAND_HOLD_MS) {
          Serial.println("  (held still for 2s - treating as a hand, capturing here)");
          handDetected(sliderVal);
          break;
        }
      }
