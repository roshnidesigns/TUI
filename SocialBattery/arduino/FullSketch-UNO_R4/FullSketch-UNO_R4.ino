/*
 * Social Battery — full sketch, Arduino UNO R4 WiFi
 ...
 * and wait to be told; a serial command (below) sets a level per arm, and the
 * level decides how wide and how fast that arm swings.
 *
 ...
 * ping-pongs between the two, paced to the same seconds-per-swing as the arms.
 * Where you leave it picks a level, measured as equal quarters either side of the
 * CENTRE of travel — so the scale is symmetric and a position and its mirror are
 * always the same level.
 *
 * An 8-pixel NeoPixel strip shows that level: the lit fill grows outward from the
 * middle pair as the level rises - nothing at Off, the middle pair at Low, widening
 * at Medium, the whole strip at High.
 *
 * ── WIRING — Arduino UNO R4 WiFi ────────────────────────────────────────────
 *
 *   D1    NeoPixel strip data         D3/D5  HW-354 driver IN1 / IN2
 *   D0    servo arm 0 (blue)          A1     fader wiper
 *   D7    servo arm 1 (orange)
 *
 *   Servo power and driver power both from an EXTERNAL 5V supply, with its
 *   ground tied to the board's. The board's own regulator cannot source motor
 *   current, and trying makes it brown out and drop off USB mid-movement.
 *
 *   This board runs at 5V (unlike the MKR WiFi 1010 this was ported from, which
 *   was 3.3V) - the fader's pot is wired to the board's 5V pin, not 3.3V. Every
 *   SLIDER_MIN/MAX/CENTER number below is specific to that 5V wiring and this
 *   physical pot; re-run k (calibrate) after any hardware change.
 *
 * ── SERIAL, 115200 ──────────────────────────────────────────────────────────
 *
 *   H                  handshake; answers OK:HELLO ... ARMS=<n>
 *   ?                  print current arm and fader status
 ...
 *  2. analogWrite() everywhere on the motor pins, never digitalWrite() - mixing
 *     the two on the same pin has caused silent failures on other Arduino cores.
...
#include <Servo.h>               // any digital pin, via a shared hardware timer
...
// D8-D10 are free general-purpose pins on this board (the onboard ESP32-S3 WiFi
// module uses its own separate header, unlike the MKR's NINA module on D8-D10).
...
// UNO R4 WiFi wiring, run at 5V (the MKR's numbers below don't apply - that board
// ran the pot at 3.3V, which reads on a completely different scale). True mechanical
// stops (from the k command) are 18/1014, pulled in by ~100 each side, same margin-
// off-the-hard-stop idea as the MKR sketch, to avoid driving flat-out into them.
#define SLIDER_MIN 118
#define SLIDER_MAX 914
...
void faderUpdate() {
  ...
  // Print status only on a real change, not on a timer - a piece left running at a
  // steady level stays quiet instead of repeating the same line every pass.
...
//
// Always analogWrite(), never digitalWrite() on these pins - mixing the two on the
// same pin has caused silent failures on other Arduino cores.
...
// Prints one human-readable status line for every arm, plus the fader - on demand
// from '?', not on a timer.
void report() {
  for (uint8_t i = 0; i < ARM_COUNT; i++) {
    Serial.print(F("D")); Serial.print(SERVO_PIN[i]);
    Serial.print(F(": ")); Serial.print(STATE_NAME[armState[i]]);
    Serial.print(armRunning[i] ? F(" running") : F(" stopped"));
    Serial.print(F(" at ")); Serial.print((int)(armAngle[i] + 0.5)); Serial.println(F(" deg"));
  }
  Serial.print(F("slider: ")); Serial.print(readSlider());
  Serial.print(F("  level: ")); Serial.print(LEVEL_NAME[faderLevel]);
  Serial.print(F("  mode: ")); Serial.println(faderPhaseName());
}
...
void handleLine(char *s) {
  ...
  if (kind == '?') { report(); return; }   // one-off status line on demand
