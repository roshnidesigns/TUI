 ...
 *
 * Based on FINAL-SPEED-FAST-FIX, with one deliberate change: the fader no longer
 * chases a virtual timed target (PACED mode). It drives continuously toward the
 * real physical end of the swing instead - like Fader_Mirror - which reads as a
 * smooth glide rather than PACED's stop-and-catch-up rhythm. The trade: hitting
 * ~30/10/6s per level is now an approximate, duty-tuned thing (SWING_SPEED), not
 * an exact clock like PACE_SECONDS was. The servo arm's own timing is untouched -
 * it still runs a hardcoded cycle on its own from the moment the board boots -
 * OFF -> MED -> HIGH, 20 seconds each, then loops back to OFF and repeats (LOW is
 * left out of the cycle, still reachable manually). Typing a manual arm command
 * (low/med/high/stop/go) still works and takes over
 * from the cycle, same as before.
 *
...
// Hardcoded demo cycle: runs the arm through OFF -> MED -> HIGH, 20s each, on
// repeat, starting the moment the board boots - no serial input needed. LOW is
// left out of the cycle entirely (still reachable manually, see below). Any
// manual arm command (low/med/high/stop/go) takes over and cancels the cycle.
enum CycleStep { CYCLE_OFF = 0, CYCLE_MED = 1, CYCLE_HIGH = 2, CYCLE_STEP_COUNT = 3 };
const char* CYCLE_NAME[CYCLE_STEP_COUNT] = { "OFF", "MED", "HIGH" };
...
void setup() {
  ...
  Serial.println("ARM auto-cycling OFF -> MED -> HIGH, 20s each, repeating (LOW not in the cycle)");
  Serial.print("travel ");
