 ...
 *
 * The fader below is untouched from SERVO+SLIDER+FinalSept24 - still hand-driven
 * (move it, let go, it mimics). The servo arm is different here: instead of
 * waiting for a low/med/high command, it runs a hardcoded cycle on its own from
 * the moment the board boots - OFF -> LOW -> MED -> HIGH, 20 seconds each, then
 * loops back to OFF and repeats. Typing a manual arm command (low/med/high/stop/
 * go) still works and takes over from the cycle, same as before.
 *
  ...
  switch (mode) {
    ...
    case PACED: {
      ...
      if (gap <= MARGIN_ERROR) {
        ...
        motorForward(driveSpeed(gap, SWING_SPEED[level]));
      } else {
        motorBackward(driveSpeed(gap, SWING_SPEED[level]));
      }
      break;
    }
  }
