/*
 * Social Battery — the motorized fader, plus one servo arm (D7), LEVEL-SYNCED
 * Tangible User Interface, CIID
 *
 * Based on FINAL-DIFF-SPEED. No hardcoded cycle here - the servo is driven BY the
 * slider instead. Set a level on the slider by hand, same as always (move it, let
 * go, it picks LOW/MEDIUM/HIGH/OFF from where you left it) - and the moment that
 * level is detected, the servo arm switches to match it (LOW/MED/HIGH/off), with
 * its phase clock reset so both start that new swing leg at the same moment. That
 * sync is a one-time alignment at the instant the level changes, not a continuous
 * shared pace - the two keep running at their own speed after that (servo's exact
 * ARM_STATE_RATE vs slider's duty-tuned SWING_SPEED). Matching pace continuously
 * is a later, separate file.
 *
 * A manual servo command (low/med/high/stop/go) still works, as a temporary
 * override - the next real slider gesture re-syncs it.
 *
 * The fader itself drives continuously toward the real physical end of the swing
 * (no virtual paced target - like Fader_Mirror, a smooth glide). Hitting ~30/10/6s
...
#define SERVO_PIN 7
#define ARM_CENTER_ANGLE 88   // resting vertical, measured by hand
...
// No hardcoded cycle here - the servo is driven BY the slider instead. Whenever
// the slider's level changes (a real hand gesture, via detectFrom() - or the
// landing level at boot), applyLevelToServo() (defined further down, once
// LEVEL_NAME is in scope) mirrors that same level onto the arm and syncs its
// phase clock, so both start their new swing leg at the same moment. See that
// function for what "sync" means here - a one-time alignment, not a continuous
// shared pace. A manual arm command (low/med/high/stop/go) still works as a
// temporary override - it holds until the next real slider gesture re-syncs it.
...
// Mirrors the slider's level (0=OFF, 1=LOW, 2=MEDIUM, 3=HIGH) onto the servo arm,
// and resets the servo's phase clock at the same time - so switching levels
// together also starts each side's swing leg together, not just matches the
// level. As in SYNC-FINAL-HARDCODE, this is a one-time alignment at the moment
// of the change, not a continuous shared pace: the two keep running at their own
// speed after that (servo's exact ARM_STATE_RATE vs slider's duty-tuned
// SWING_SPEED) - matching pace continuously is a later, separate file.
void applyLevelToServo(int lvl) {
  armPhase = 0;
  switch (lvl) {
  ...
  Serial.print("ARM synced to slider level: ");
...
void detectFrom(int x) {
  ...
  int prevLevel = level;   // so the servo only re-syncs on a REAL level change, not every touch
  ...
  // Only re-sync the servo on a genuine level change - re-syncing on every touch,
  // even ones that kept the same level, would restart the arm's swing leg (and
  // visibly jump) for no reason.
  if (level != prevLevel) applyLevelToServo(level);
...
void handleCommand(char* cmd) {
  ...
  if (eqIgnoreCase(cmd, "c")) {
    ...
    startArm(ARM_LOW);   // temporary override - the next real slider gesture re-syncs it
...
void setup() {
  ...
  Serial.println("ARM synced to whatever level you set on the slider by hand");
  Serial.print("travel ");
  ...
  switch (mode) {
    ...
    case HOMING: {
      ...
      if (abs(SLIDER_CENTER - sliderVal) <= MARGIN_ERROR || timedOut) {   // close enough, or out of time
        ...
        applyLevelToServo(level);                  // sync the servo to match, at boot too
