...
* The slider drives itself to fixed positions stored in a position array,
* advancing to the next entry the moment it arrives - so with this array it
* ping-pongs between 400 and 600 continuously, never resting.
*
...
// Where in the array to start. The index advances on its own each time a
// position is reached, so this is only the starting point: 1 = seek 600 first.
int currentTarget = 1;
